...
app.get('localhost:3001/api/health', (req, res) => {
...
// --- Endpoint Coperto ---
app.get('localhost:3001/api/coperto', (req, res) => {
  console.log('📋 Richiesta coperto ricevuta');
  res.json(coperto);
});

app.post('localhost:3001/api/coperto', (req, res) => {
...
app.get('localhost:3001/api/ordini', (req, res) => {
  try {
...
app.get('localhost:3001/api/ordini/completo', (req, res) => {
  try {
...
app.get('localhost:3001/api/ordini/tavolo/:tavolo', (req, res) => {
  try {
...
app.post('localhost:3001/api/ordina', (req, res) => {
  try {
    let ordini = leggiFileSicuro(FILE_PATH);
...
app.post('localhost:3001/api/ordini/:id/evaso', (req, res) => {
  try {
...
app.delete('localhost:3001/api/ordini/tavolo/:tavolo', (req, res) => {
  try {
    const tavolo = req.params.tavolo;
...
app.post('localhost:3001/api/tavoli/:tavolo/stampa-totale', (req, res) => {
  try {
    const tavolo = req.params.tavolo;
...
// --- Tavoli occupati ---
app.get('localhost:3001/api/tavoli/occupati', (req, res) => {
  try {
...
app.post('localhost:3001/api/tavoli/occupa', (req, res) => {
  try {
...
app.post('localhost:3001/api/tavoli/libera', (req, res) => {
  try {
    const { tavolo } = req.body;
    if (!tavolo) {
      return res.status(400).json({ message: 'Table number missing' });
...
// --- Menu ---
app.get('localhost:3001/api/menu', (req, res) => {
  try {
...
app.post('localhost:3001/api/menu', (req, res) => {
  try {
...
// --- Modifica prodotto ---
app.put('localhost:3001/api/menu/:id', (req, res) => {
  try {
    const id = parseInt(req.params.id);
    let menu = leggiFileSicuro(MENU_FILE_PATH);
...
// --- Elimina prodotto ---
app.delete('localhost:3001/api/menu/:id', (req, res) => {
  try {
    const id = parseInt(req.params.id);
    let menu = leggiFileSicuro(MENU_FILE_PATH);
...
// --- Elimina categoria ---
app.delete('localhost:3001/api/categoria/:categoria', (req, res) => {
  try {
...
app.post('localhost:3001/api/ordini/pulizia-forzata', (req, res) => {
  try {
...
// SERVER PER RENDER - RISTORANTE BELLAVISTA
const express = require('express');
const cors = require('cors');
const fs = require('fs');
const path = require('path');
...
// ✅ PATH UNIVERSALI per Render
const getFilePath = (filename) => {
  return path.join(__dirname, filename);
};

const FILE_PATH = getFilePath('ordini.json');
const MENU_FILE_PATH = getFilePath('menu.json');
const TAVOLI_OCCUPATI_PATH = getFilePath('tavoliOccupati.json');
const COPERTI_PATH = getFilePath('coperto.json');

// ✅ INIZIALIZZAZIONE SICURA
function inizializzaFile() {
  const files = [
    { path: FILE_PATH, default: '[]' },
    { path: MENU_FILE_PATH, default: '[]' },
    { path: TAVOLI_OCCUPATI_PATH, default: '[]' },
    { path: COPERTI_PATH, default: '{"attivo":true,"prezzo":2.0}' }
  ];
  ...
  files.forEach(file => {
    try {
      if (!fs.existsSync(file.path)) {
        ...
        console.log(`✅ Creato: ${path.basename(file.path)}`);
      } else {
        const content = fs.readFileSync(file.path, 'utf8');
        console.log(`✅ File esistente: ${path.basename(file.path)} (${content.length} chars)`);
      }
    } catch (error) {
      console.error(`❌ Errore file ${path.basename(file.path)}:`, error.message);
    }
  });
}
...
// --- Funzioni helper SICURE per file JSON ---
function leggiFileSicuro(filePath) {
  try {
    if (!fs.existsSync(filePath)) {
      console.log(`⚠️ File non trovato, ricreo: ${path.basename(filePath)}`);
      const defaultContent = filePath === COPERTI_PATH ? '{"attivo":true,"prezzo":2.0}' : '[]';
    ...
    if (data.trim() === '') {
      console.log(`⚠️ File vuoto, ricreo: ${path.basename(filePath)}`);
      const defaultContent = filePath === COPERTI_PATH ? '{"attivo":true,"prezzo":2.0}' : '[]';
      fs.writeFileSync(filePath, defaultContent);
      return JSON.parse(defaultContent);
    }
    ...
    console.error(`❌ Errore lettura ${path.basename(filePath)}:`, error.message);
    const defaultContent = filePath === COPERTI_PATH ? '{"attivo":true,"prezzo":2.0}' : '[]';
    fs.writeFileSync(filePath, defaultContent);
    return JSON.parse(defaultContent);
  }
...
function scriviFileSicuro(filePath, data) {
  try {
    fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
    return true;
  } catch (error) {
    console.error(`❌ Errore scrittura ${path.basename(filePath)}:`, error.message);
    return false;
  }
}
...
function occupaTavolo(tavolo) {
  try {
    ...
    if (!occupati.includes(tavolo)) {
      occupati.push(tavolo);
      scriviFileSicuro(TAVOLI_OCCUPATI_PATH, occupati);
      console.log(`✅ Tavolo ${tavolo} occupato`);
    }
  } catch (error) {
    console.error('❌ Errore occupaTavolo:', error);
  }
...
function liberaTavolo(tavolo) {
  try {
    ...
    occupati = occupati.filter(t => t.toString() !== tavolo.toString());
    scriviFileSicuro(TAVOLI_OCCUPATI_PATH, occupati);
    console.log(`✅ Tavolo ${tavolo} liberato`);
  } catch (error) {
    console.error('❌ Errore liberaTavolo:', error);
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'OK',
    server: 'QRCode Ristorante - Render',
...
app.get('/api/ordini', (req, res) => {
  try {
    const ordini = leggiFileSicuro(FILE_PATH);
    const ordiniAttivi = ordini.filter(ordine => ordine.stato !== 'chiuso');
    res.json(ordiniAttivi);
  } catch (error) {
    res.status(500).json({ error: 'Errore lettura ordini' });
  }
...
app.post('/api/test-order', (req, res) => {
  try {
    ...
    const testOrder = {
      ...
      tavolo: "TEST",
      ordinazione: [
        { prodotto: "Pizza Margherita", quantità: 1, prezzo: 8.00 },
        { prodotto: "Acqua Naturale", quantità: 2, prezzo: 2.00 }
      ],
      stato: "in_attesa",
      timestamp: new Date().toISOString(),
    ...
    ordini.push(testOrder);
    
    if (scriviFileSicuro(FILE_PATH, ordini)) {
      res.json({ 
        success: true, 
        message: 'Ordine test creato',
        order: testOrder 
      });
    } else {
...
app.get('/', (req, res) => {
  res.json({
    message: 'Server QRCode Ristorante - Benvenuto!',
    deployedOn: 'Render',
    endpoints: {
      health: '/api/health',
...
app.listen(PORT, () => {
  console.log('🚀 SERVER QRCODE RISTORANTE - RENDER');
  console.log('📍 Porta:', PORT);
  console.log('🌐 Ambiente:', process.env.NODE_ENV || 'production');
  console.log('📁 Directory:', __dirname);
  console.log('✅ File inizializzati:');
  console.log('   📋 ordini.json');
  console.log('   🍕 menu.json');
  console.log('   🪑 tavoliOccupati.json');
  console.log('   💰 coperto.json');
});
